 ...
 *   - MXI (global materials)         ← mining producers (10%)
 *   - SPY (broad US)                 ← catch-all (28%: utilities, ag, residual)
 *
 * Pesos: NNLS con Σw=1 y w≥0, fitteado en overlap 2021-02 → 2026-04.
 * Train/holdout split: 50/13 para reportar tracking error out-of-sample.
 *
 * Output:
...
async function main() {
  // 1. Leer CSV: IXC, MXI, SPY ya están. INFL también (63 reales + 181 NaN).
  console.log('[step 1] Leyendo CSV existente…');
  const { header, rows } = readCsv(CSV_PATH);
  const ixcRet = getCol(header, rows, 'IXC');
  const mxiRet = getCol(header, rows, 'MXI');
  const spyRet = getCol(header, rows, 'SPY');
  const inflReal = getCol(header, rows, 'INFL');
  console.log(`  IXC: ${ixcRet.size} meses · MXI: ${mxiRet.size} · SPY: ${spyRet.size} · INFL real: ${inflReal.size}`);
  ...
  // Overlap = meses donde TODOS los 5 + INFL real tienen valor
  const allYms = [...inflReal.keys()].sort();
  const overlapYms = allYms.filter((ym) =>
    ixcRet.has(ym) && mxiRet.has(ym) && spyRet.has(ym) && gdxRet.has(ym) && iaiRet.has(ym)
  );
  ...
  const componentNames = ['IXC', 'GDX', 'IAI', 'MXI', 'SPY'];
  const components = [ixcRet, gdxRet, iaiRet, mxiRet, spyRet];
  ...
  // 7. Computar serie sintética COMPLETA (2006-01 → 2026-04 si data disponible)
  console.log('[step 6] Computando serie sintética completa…');
  const allCsvYms = rows.map((r) => r[0]);
  const synthetic = new Map();
  const proxyAvail = (ym) =>
    ixcRet.has(ym) && mxiRet.has(ym) && spyRet.has(ym) && gdxRet.has(ym) && iaiRet.has(ym);
  let nSynth = 0, nMissing = 0;
  for (const ym of allCsvYms) {
    if (proxyAvail(ym)) {
      const ret = w[0] * ixcRet.get(ym) + w[1] * gdxRet.get(ym) + w[2] * iaiRet.get(ym) +
                  w[3] * mxiRet.get(ym) + w[4] * spyRet.get(ym);
  ...
  console.log(`  Meses con proxy disponible: ${nSynth} · sin proxy (todos componentes): ${nMissing}`);
